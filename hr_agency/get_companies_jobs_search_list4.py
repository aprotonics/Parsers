...
for i in range(companies_list_length):
    ...
    if type(company_formatted) == type([]):
        ...
        company_formatted_list = []
        company_list = company_formatted_str.split(" ")
        for value in company_list:
            value_formatted = value.strip()
            company_formatted_list.append(value_formatted)
        
        company_formatted_str = ""
        for value in company_formatted_list:
            if value:
                company_formatted_str += value
                company_formatted_str += " "
